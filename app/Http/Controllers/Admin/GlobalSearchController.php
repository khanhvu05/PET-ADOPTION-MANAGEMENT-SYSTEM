...
class GlobalSearchController extends Controller
{
    public function search(Request $request)
    {
        ...
        // 1. Search Pets
        $pets = Pet::where('Ten_thu_cung', 'like', "%{$q}%")
            ->orWhere('Giong', 'like', "%{$q}%")
            ->orWhere('Mau_sac', 'like', "%{$q}%")
            ->limit(5)->get();
            
        foreach($pets as $pet) {
            $results[] = [
                'type' => 'Thú cưng',
                'title' => $pet->Ten_thu_cung . ' (' . $pet->Giong . ')',
        ...
        // 3. Search Adoptions
        $adoptions = AdoptionApplication::where('id', 'like', "%{$q}%")
        ...
        foreach($adoptions as $adoption) {
            $results[] = [
                'type' => 'Đơn nhận nuôi',
                'title' => 'Đơn #' . $adoption->id . ' - ' . $adoption->Trang_thai,
                'url' => route('admin.adoptions.show', $adoption->id),

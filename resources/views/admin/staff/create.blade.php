<x-admin-layout>
    <x-slot name="header">
        ...
        <span class="text-slate-700 font-bold">Thêm Nhân Viên</span>
    </x-slot>

    <div class="max-w-7xl mx-auto space-y-6">
        <div>
            <h2 class="text-2xl font-bold text-slate-900 mb-1">Thêm Nhân Viên Mới</h2>
            <p class="text-sm text-slate-500">Tạo tài khoản và cấp quyền cho nhân viên ban quản trị.</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.staff.store') }}" class="space-y-6">
            ...
            <div class="bg-white border border-slate-200 rounded-xl shadow p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="Ho_ten" value="{{ old('Ho_ten') }}" required class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none @error('Ho_ten') border-red-500 @enderror">
                    ...
                    <div>
                        ...
                        <input type="email" name="Email" value="{{ old('Email') }}" required class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none @error('Email') border-red-500 @enderror">
                        @error('Email') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-2">Mật khẩu <span class="text-red-500">*</span></label>
                        <input type="password" name="Mat_khau_hash" required class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none @error('Mat_khau_hash') border-red-500 @enderror">
                        @error('Mat_khau_hash') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-2">Xác nhận mật khẩu <span class="text-red-500">*</span></label>
                        <input type="password" name="Mat_khau_hash_confirmation" required class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-2">Số điện thoại</label>
                        <input type="text" name="So_dien_thoai" value="{{ old('So_dien_thoai') }}" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-2">Chức danh / Vai trò hiển thị</label>
                        <input type="text" name="Chuc_danh" value="{{ old('Chuc_danh') }}" placeholder="VD: Nhân viên quản lý đơn, Nhân viên chăm sóc thú cưng..." class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none">
                ...
                <div class="mb-6">
                    ...
                    <select name="custom_role_id" class="w-full md:w-1/2 px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none">
                        ...
                        @foreach($customRoles as $role)
                            <option value="{{ $role->id }}" {{ old('custom_role_id') == $role->id ? 'selected' : '' }}>{{ $role->name }}</option>
                        @endforeach
                    </select>
                    <p class="text-xs text-slate-500 mt-1">Lưu ý: Bạn có thể chọn một Preset, sau đó vẫn có thể đánh dấu thêm các quyền bổ sung ở dưới cho cá nhân này.</p>
                </div>
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit" class="px-5 py-2.5 bg-teal-600 text-white rounded-xl hover:bg-teal-700 transition font-medium shadow-sm">Tạo Nhân Viên</button>
            </div>
        </form>
    </div>

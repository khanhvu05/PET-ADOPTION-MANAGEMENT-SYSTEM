<x-admin-layout>
    ...
    <div x-data="{ activeTab: 'details' }" class="max-w-7xl mx-auto space-y-6 lg:space-y-8 font-sans" x-cloak>
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
            ...
            <div class="flex flex-wrap items-center gap-3">
                <a href="{{ route('admin.adoptions.index') }}" class="px-4 py-2 border border-slate-200 text-slate-600 font-medium text-sm rounded-[10px] hover:bg-slate-50 transition-colors flex items-center gap-2 active:scale-90">
            ...
            <div class="flex-1 space-y-6 min-w-0">
                ...
                <div>
                    ...
                    <div x-show="activeTab === 'details'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-6">
                        ...
                        <div class="bg-white border border-slate-200 rounded-[10px] p-6" x-data="{ isEditing: {{ request()->query('edit') ? 'true' : 'false' }} }">
                            <div class="flex justify-between items-center mb-6 border-b border-slate-100 pb-3">
                                <h3 class="text-base font-semibold text-slate-800">
                                    Thông tin người nhận nuôi
                                </h3>
                                <button type="button" @click="isEditing = !isEditing" class="text-sm font-medium text-sidebar-blue hover:text-blue-700 transition-colors flex items-center gap-1.5" x-show="!isEditing">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    Chỉnh sửa
                                </button>
                            </div>
                            
                            <!-- Form tích hợp ngay trong layout cũ -->
                                ...
                                <div class="grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-6">
                                    ...
                                    <div class="space-y-5">
                                        <div class="flex items-center">
                                            ...
                                            <span x-show="!isEditing" class="flex-1 text-slate-800 font-medium text-sm">{{ $application->Ho_ten }}</span>
                                            <div x-show="isEditing" x-cloak class="flex-1">
                                                <input type="text" name="Ho_ten" value="{{ old('Ho_ten', $application->Ho_ten) }}" required class="w-full rounded-[8px] border-slate-200 shadow-sm focus:border-sidebar-blue focus:ring-sidebar-blue text-sm px-3 py-1.5 font-medium text-slate-800">
                                            </div>
                                        </div>
                                        <div class="flex items-center">
                                            <span class="text-slate-500 font-medium text-[13px] flex items-center gap-2 w-[140px] shrink-0">
                                            ...
                                            <!-- Email không cho phép sửa, vì nó dính tới tài khoản -->
                                        </div>
                                        <div class="flex items-center">
                                            <span class="text-slate-500 font-medium text-[13px] flex items-center gap-2 w-[140px] shrink-0">
                                            ...
                                            <span x-show="!isEditing" class="flex-1 text-slate-800 font-medium text-sm">{{ $application->So_dien_thoai }}</span>
                                            <div x-show="isEditing" x-cloak class="flex-1">
                                                <input type="tel" name="So_dien_thoai" value="{{ old('So_dien_thoai', $application->So_dien_thoai) }}" required class="w-full rounded-[8px] border-slate-200 shadow-sm focus:border-sidebar-blue focus:ring-sidebar-blue text-sm px-3 py-1.5 font-medium text-slate-800">
                                            </div>
                                        </div>
                                        <div class="flex items-start pt-1">
                                            <span class="text-slate-500 font-medium text-[13px] flex items-center gap-2 w-[140px] shrink-0 mt-1">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                                                Địa chỉ
                                            </span>
                                            <span x-show="!isEditing" class="flex-1 text-slate-800 font-medium text-sm leading-relaxed mt-1">{{ $application->Dia_chi }}</span>
                                            <div x-show="isEditing" x-cloak class="flex-1">
                                                <input type="text" name="Dia_chi" value="{{ old('Dia_chi', $application->Dia_chi) }}" required class="w-full rounded-[8px] border-slate-200 shadow-sm focus:border-sidebar-blue focus:ring-sidebar-blue text-sm px-3 py-1.5 font-medium text-slate-800">
                                            </div>
                                        </div>
                                    </div>
                                    <!-- Cột Phải -->
                                    <div class="space-y-5">
                                        <div class="flex items-center">
                                            ...
                                            <span x-show="!isEditing" class="flex-1 text-slate-800 font-medium text-sm">{{ $application->Nghe_nghiep }}</span>
                                            <div x-show="isEditing" x-cloak class="flex-1">
                                                <input type="text" name="Nghe_nghiep" value="{{ old('Nghe_nghiep', $application->Nghe_nghiep) }}" class="w-full rounded-[8px] border-slate-200 shadow-sm focus:border-sidebar-blue focus:ring-sidebar-blue text-sm px-3 py-1.5 font-medium text-slate-800">
                                            </div>
                                        </div>
                                        <div class="flex items-center">
                                            <span class="text-slate-500 font-medium text-[13px] flex items-center gap-2 w-[140px] shrink-0">
                                            ...
                                            <span x-show="!isEditing" class="flex-1 text-slate-800 font-medium text-sm">{{ $application->Loai_nha_o }}</span>
                                            <div x-show="isEditing" x-cloak class="flex-1">
                                                <select name="Loai_nha_o" class="w-full rounded-[8px] border-slate-200 shadow-sm focus:border-sidebar-blue focus:ring-sidebar-blue text-sm px-3 py-1.5 font-medium text-slate-800">
                                        ...
                                        <div class="flex items-center">
                                            ...
                                            <span class="flex-1 text-slate-800 font-medium text-sm">Có cam kết chăm sóc thú cưng</span>
                                        </div>
                                    </div>
                                </div>
                                
                                <div class="flex justify-end gap-3 pt-5 mt-5 border-t border-slate-100" x-show="isEditing" x-cloak>
                                    <button type="button" @click="isEditing = false" class="px-4 py-2 border border-slate-200 text-slate-600 font-medium text-sm rounded-[8px] hover:bg-slate-50 transition-colors active:scale-95">
                                        Hủy bỏ
                                    </button>
                                    <button type="submit" class="px-4 py-2 bg-sidebar-blue text-white font-medium text-sm rounded-[8px] hover:opacity-90 transition-colors shadow-sm active:scale-95">
                                        Lưu thay đổi
                                    </button>
                                </div>
                            </form>
                        ...
                        <div class="bg-white border border-slate-200 rounded-[10px] p-6">
                            ...
                            <div class="space-y-4">
                                @if($application->answers && $application->answers->count() > 0)
                                        ...
                                        <div class="p-4 border border-slate-100 rounded-[10px]">
                                            ...
                                            <div class="bg-slate-50 p-3 rounded-[8px] border border-slate-100">
                                                <p class="text-sm text-slate-700">
                                                    @if(!empty($answer->Lua_chon_da_chon))
                                                        @if(is_array($answer->Lua_chon_da_chon))
                                                            {{ implode(', ', $answer->Lua_chon_da_chon) }}
                                                        @else
                                                            {{ $answer->Lua_chon_da_chon }}
                                                        @endif
                                                    @elseif(!empty($answer->Noi_dung_tra_loi))
                                                        {!! nl2br(e($answer->Noi_dung_tra_loi)) !!}
                                                    @else
                                    ...
                                    <!-- Hiển thị thêm Lý do nhận nuôi (Khớp với form khách hàng) -->
                                    <div class="p-4 border border-slate-100 rounded-[10px]">
                                        <p class="text-sm font-semibold text-slate-800 mb-2">{{ $qCount + 1 }}. Lý do bạn muốn nhận nuôi bé {{ $application->thuCung->Ten ?? 'thú cưng' }}?</p>
                                        <div class="bg-slate-50 p-3 rounded-[8px] border border-slate-100">
                                            <p class="text-sm text-slate-700">
                                                {!! nl2br(e($application->Ly_do_nhan_nuoi)) !!}
                                            </p>
                                        </div>
                                    </div>
                                @else
                            ...
                            @if($application->Ghi_chu_admin)
                                <div class="flex items-start gap-4">
                                    ...
                                    <div class="flex-1 bg-slate-50 p-4 rounded-[10px] border border-slate-100">
                                        ...
                                        <p class="text-sm text-slate-600">{!! nl2br(e($application->Ghi_chu_admin)) !!}</p>
                                    </div>
                                </div>
                            @else
                ...
                <div class="bg-white border border-slate-200 rounded-[10px] p-6 sticky top-6">
                    ...
                    <form id="action-form" action="{{ route('admin.adoptions.update', $application->Ma_don) }}" method="POST" class="space-y-3">
                        ...
                        @if($application->Trang_thai === 'cho_duyet')
                            <button type="button" onclick="confirmAction('cho_xac_nhan_don', 'Hệ thống sẽ gửi email mời phỏng vấn đến ứng viên. Ứng viên có 24h để chọn lịch. Bạn có chắc chắn?')" class="w-full bg-sidebar-blue hover:bg-sidebar-blue/80 text-white font-medium text-sm py-2.5 rounded-[8px] transition-colors active:scale-90">
                            ...
                            <div class="mt-4 border-t border-slate-100 pt-4">
                                ...
                                @enderror
                                <button type="button" onclick="confirmReject()" class="w-full mt-2 bg-[#E75E5B] hover:bg-red-400 text-white font-medium text-sm py-2.5 rounded-[8px] transition-colors active:scale-90">
                                    Từ chối đơn
                                </button>
                            </div>
                        @endif
                    ...
                    <div class="mt-6 pt-5 border-t border-slate-100">
                        ...
                        <form action="{{ route('admin.adoptions.update', $application->Ma_don) }}" method="POST" id="admin-notes-form">
                            @csrf
                            @method('PUT')
                            <input type="hidden" name="Trang_thai" value="{{ $application->Trang_thai }}">
                            <textarea name="Ghi_chu_admin" rows="3" class="w-full bg-slate-50 border border-slate-200 rounded-[8px] px-3 py-2 text-sm text-slate-700 focus:outline-none focus:border-teal-500 transition-colors" placeholder="Thêm ghi chú mới..."></textarea>
                            <button type="submit" id="btn-save-notes" class="w-full mt-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium text-sm rounded-[8px] py-2 transition-colors active:scale-90 flex justify-center items-center gap-2">
    ...
    <script>
        function confirmAction(status, textMessage) {
            ...
            let iconColor = 'text-sidebar-blue';
            let iconRing = 'ring-slate-100';
            let btnClass = 'bg-sidebar-blue hover:opacity-90 focus:ring-slate-200';
            let iconSvg = '<circle cx="12" cy="12" r="10" stroke-width="2"></circle><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 16v-4"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8h.01"></path>';
            let titleText = 'Xác nhận xử lý';

            if(status === 'rejected') {
                iconBg = 'bg-red-50';
                iconColor = 'text-red-600';
                iconRing = 'ring-red-50/50';
                btnClass = 'bg-[#E75E5B] hover:bg-[#d9524f] focus:ring-red-100';
                iconSvg = '<circle cx="12" cy="12" r="10" stroke-width="2"></circle><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m15 9-6 6"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m9 9 6 6"></path>';
                titleText = 'Xác nhận từ chối';
            } else if(status === 'completed') {
                iconBg = 'bg-purple-50';
                iconColor = 'text-purple-600';
                iconRing = 'ring-purple-50/50';
                btnClass = 'bg-purple-600 hover:bg-purple-700 focus:ring-purple-100';
                iconSvg = '<circle cx="12" cy="12" r="10" stroke-width="2"></circle><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m9 12 2 2 4-4"></path>';
                titleText = 'Hoàn tất bàn giao';
            } else if(status === 'cho_xac_nhan_don') {
                iconBg = 'bg-slate-50';
                iconColor = 'text-sidebar-blue';
                iconRing = 'ring-slate-100';
                btnClass = 'bg-sidebar-blue hover:opacity-90 focus:ring-slate-200';
                iconSvg = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m22 2-7 20-4-9-9-4Z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M22 2 11 13"></path>';
                titleText = 'Gửi thư mời phỏng vấn';
            }

            Swal.fire({
                html: `
                    <div class="flex flex-col items-center pt-4 pb-2">
                        <div class="w-16 h-16 ${iconBg} ${iconColor} rounded-full flex items-center justify-center mb-5 shadow-sm ring-4 ${iconRing}">
                            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                ${iconSvg}
                            </svg>
                        </div>
                        <h3 class="text-xl font-bold text-slate-800 mb-2">${titleText}</h3>
                        <p class="text-[14px] text-slate-500 text-center leading-relaxed px-2">
                            ${textMessage}
                        </p>
                    </div>
                `,
                ...
                customClass: {
                    ...
                    confirmButton: `${btnClass} text-white font-semibold text-sm px-8 py-2.5 rounded-[10px] transition-all shadow-md min-w-[120px]`,
                    cancelButton: 'bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-sm px-8 py-2.5 rounded-[10px] transition-all min-w-[120px]'
                }
            }).then((result) => {
                if (result.isConfirmed) {
                    Swal.fire({
                        title: 'Đang xử lý...',
                        text: 'Vui lòng đợi trong giây lát. Hệ thống đang thực hiện...',
        ...
        function confirmReject() {
            let textarea = document.getElementById('ghi_chu_reject');
            let note = textarea.value.trim();
            let parentDiv = textarea.parentElement;

            if(!note) {
                textarea.classList.remove('border-slate-200', 'focus:border-red-500');
                textarea.classList.add('border-red-500', 'ring-1', 'ring-red-500');
                
                let existingError = parentDiv.querySelector('.reject-error-msg');
                if (!existingError) {
                    let errorNode = document.createElement('p');
                    errorNode.className = 'reject-error-msg text-red-500 text-xs mt-1';
                    errorNode.innerText = 'Vui lòng nhập lý do từ chối trước khi thực hiện.';
                ...
                textarea.addEventListener('input', function removeError() {
                    if (this.value.trim() !== '') {
                        ...
                        let err = parentDiv.querySelector('.reject-error-msg');
                        if (err) err.remove();
            ...
            document.getElementById('trang_thai_input').value = 'tu_choi';
            document.getElementById('action-form').submit();
        }

        document.getElementById('admin-notes-form').addEventListener('submit', function(e) {
            e.preventDefault();
            let form = this;
            let textarea = form.querySelector('textarea[name="Ghi_chu_admin"]');
            let noteContent = textarea.value.trim();

            if (!noteContent) {
                textarea.classList.remove('border-slate-200', 'focus:border-teal-500');
                textarea.classList.add('border-red-500', 'focus:border-red-500', 'ring-1', 'ring-red-500');
                
                let existingError = form.querySelector('.note-error-msg');
                if (!existingError) {
                    let errorNode = document.createElement('p');
                    errorNode.className = 'note-error-msg text-red-500 text-xs mt-1';
                    errorNode.innerText = 'Vui lòng nhập nội dung ghi chú trước khi lưu.';
                    textarea.parentNode.insertBefore(errorNode, textarea.nextSibling);
                }
                
                textarea.addEventListener('input', function removeError() {
                    if (this.value.trim() !== '') {
                        this.classList.remove('border-red-500', 'focus:border-red-500', 'ring-1', 'ring-red-500');
                        this.classList.add('border-slate-200', 'focus:border-teal-500');
                        let err = form.querySelector('.note-error-msg');
                        if (err) err.remove();
                        this.removeEventListener('input', removeError);
                    }
                });
            ...
            let btn = document.getElementById('btn-save-notes');
            ...
            .then(data => {
                ...
                if(data.success) {
                    ...
                    // Update the note history tab
                    let noteTab = document.querySelector('[x-show="activeTab === \'notes\'"] .space-y-4');
                    if (noteTab) {
                        if (data.ghi_chu) {
                            noteTab.innerHTML = `
                                <div class="flex items-start gap-4">
                                    ...
                                    <div class="flex-1 bg-slate-50 p-4 rounded-[10px] border border-slate-100">
                                        <div class="flex justify-between mb-2">
                                            <p class="text-[13px] font-semibold text-slate-700">Admin</p>
                                            <p class="text-[11px] text-slate-400">${data.timestamp}</p>
                                        </div>
                                        <p class="text-sm text-slate-600">${data.ghi_chu.replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\n/g, "<br>")}</p>
                                    </div>
                                </div>
                                <div class="mt-4 pt-4 border-t border-slate-100">
                                    <p class="text-xs text-slate-500 italic">Để thêm hoặc cập nhật ghi chú, vui lòng sử dụng khung "Ghi chú từ Admin" ở cột bên phải.</p>
            ...
            .catch(error => {
                btn.innerHTML = originalText;
                btn.disabled = false;
                window.dispatchEvent(new CustomEvent('notify', { detail: { type: 'error', message: 'Không thể lưu ghi chú lúc này.' }}));
            });
        });
    </script>
    @endpush
